...
@dataclass
class JinrishiciSentenceResponse:
    error: str | None = None
    title: str = ""
    sequence: str = ""
    author: str = ""
...
class HTTPService:
            ...
            async with self.session.get(settings.jinrishici_api_endpoint + "/sentence", headers=headers) as resp:
                if resp.status == 200:
                    ...
                    if res["status"] == "success":
                        return JinrishiciSentenceResponse(
                            title=res["data"]["origin"]["title"],
                            sequence=res["data"]["content"],
                            author=res["data"]["origin"]["author"],
                            content="\n".join(res["data"]["origin"]["content"]),
                            translate="".join(res["data"]["origin"]["translate"] or ""),
                        )
                return JinrishiciSentenceResponse(error=f"HTTP {resp.status} {resp.reason}")
        except Exception as e:
            return JinrishiciSentenceResponse(error=repr(e))
    ...
    async def fetch_feed(self, url: str) -> FetchFeedResponse:
        try:
            ...
            return FetchFeedResponse(error=repr(e))
